(* Eric Song & Chang-Woo Ham Section A
...
and
  ...
  | Record(fs) ->
    ...
    | Proj(e,id) ->
      eval_expr e >>= fields_of_recordVal >>= fun flist ->
      (match List.assoc_opt id flist with
      | None -> error "field does not exist"
      | Some(false, ev) -> (return ev)
      | Some(true, RefVal ev) -> (Store.deref g_store ev)
      | _ -> error "Error")
    | SetField(e1,id,e2) ->
      eval_expr e1 >>=
      fields_of_recordVal >>= fun flist ->
      eval_expr e2 >>= fun target ->
      (match List.assoc_opt id flist with 
      | None -> error "field does not exist"
      | Some(true, RefVal ev) -> (Store.set_ref g_store ev target) >>= fun final ->
        return UnitVal

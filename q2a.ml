...
let rec level t i = 
   match i,t with
   | 1,Node(d,_,_) -> [d]
   | _,Node(d, Empty,rt) -> (level rt (i-1))
   | _,Node(d,lt, Empty) -> (level lt (i-1))
   | _,Node(d,lt,rt) -> (level lt (i-1))@(level rt (i-1))
...
let rec levels t =
   let rec levels_helper t cl =
      match cl with
      | 0 -> []
      | n -> level t n :: levels_helper t (n-1)
   in levels_helper t (height t)
...
let rec pbt : int -> 'a -> 'a btree = 
   fun h d ->
   match h with
   | 0 -> Empty
   | 1 -> Node(d, Empty, Empty)
